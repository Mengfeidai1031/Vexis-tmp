@extends('layouts.app')
@section('title', 'Crear Festivo - VEXIS')
@section('content')
<div class="vx-page-header">
    <h1 class="vx-page-title">Crear Nuevo Festivo</h1>
    ...
    <div class="vx-card"><div class="vx-card-body">
        <form action="{{ route('festivos.store') }}" method="POST">
            ...
            <div class="vx-form-group">
                ...
                <input type="text" class="vx-input @error('nombre') is-invalid @enderror" id="nombre" name="nombre" value="{{ old('nombre') }}" required placeholder="Ej: San Juan">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 16px;">
                <div class="vx-form-group">
                    ...
                    <input type="date" class="vx-input @error('fecha') is-invalid @enderror" id="fecha" name="fecha" value="{{ old('fecha') }}" required>
                ...
                <div class="vx-form-group">
                    ...
                    <select class="vx-select @error('ambito') is-invalid @enderror" id="ambito" name="ambito" required>
                        @foreach(\App\Models\Festivo::$ambitos as $k => $v)
                            <option value="{{ $k }}" {{ old('ambito') == $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                    @error('ambito')<div class="vx-invalid-feedback">{{ $message }}</div>@enderror
                </div>
            </div>
            <div class="vx-form-group" id="municipioGroup">
                <label class="vx-label" for="municipio">Municipio</label>
                <input type="text" class="vx-input" id="municipio" name="municipio" value="{{ old('municipio') }}" placeholder="Ej: Las Palmas de Gran Canaria">
                <div class="vx-form-hint">Solo para festivos locales. Déjalo vacío para nacionales o autonómicos.</div>
            </div>
...
document.getElementById('ambito').dispatchEvent(new Event('change'));
</script>
@endpush
@endsection
